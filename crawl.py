...
def process_page(page, twitter_handle, twitter_id,friend):
    ...
    if friend:
        for user in page:
            if not repository.check_in_new_waitlist_connections(user._json['screen_name'], user._json['id'],twitter_handle,twitter_id):
                repository.insert_to_new_waitlist_connections(user._json['screen_name'], user._json['id'],twitter_handle,twitter_id)
            counter += 1
    else:
        for user in page:
            if not repository.check_in_new_waitlist_connections(twitter_handle,twitter_id,user._json['screen_name'], user._json['id']):
